...
export default {
  components:{dbData, uploads, dbUsers, recentUploads, downloadExcel},
  data() {
    return {
        fileDetails: '',
        fetchedData: false,
  ...
  methods:{
    ...
    async fetchUser (data) {
        ...
        userAssign (id) {
            try {
                ...
                variables: {
                    assignInput: {
                        Id: id,
                        AssignTo: this.userData.email,
                        AssignBy: localStorage.getItem("Email"),
                        AssignOn: Date.now(),
                        ReportBy: this.reportDate,
                    }
                }
                })
                .then((response) => {
                ...
                this.assignedCount = this.assignedCount + response.data.assignData.modifiedCount
        ...
        async setNewAssign () {
            ...
            await this.assignData().then(async (response) => {
                try {
                    console.log(response)
                    await this.$apollo.mutate({
                    mutation: gql`
                    mutation (
                        $assignedEntriesInput: AssignedEntriesInput
                    ) {
                        saveAssigned (
                            assignedEntriesInput: $assignedEntriesInput
                        ) {
                    ...
                    `,
                    variables: {
                        assignedEntriesInput: {
                            UploadName: this.fileDetails.UploadName,
                            FileName: this.fileDetails.FileName,
                            Entries: response,
                            AssignedTo: this.userData.email,
                            AssignedBy: localStorage.getItem("Email"),
                            AssignedOn: Date.now(),
                            ReportingBy: this.reportDate
                        }
                    }
                    })
                    .then((response) => {
        ...
        async assignData() {
            let generatedResponse = []
                for(let item of this.selected) {
                    try {
                    var count = await this.userAssign(item)
                    generatedResponse.push(count)
                    } catch (error) {
                    console.log('error'+ error);
                    }
                }
                console.log('complete all') // gets loged first
                console.log(generatedResponse.length)
                return generatedResponse.length
        }
    },
    computed: {
        rows() {
